...
func TestToShareURLSupportsIPv6Address(t *testing.T) {
	raw := "vless://12345678-1234-1234-1234-123456789abc@[2001:db8::1]:443?encryption=none&security=tls&sni=example.com&type=xhttp&path=%2Fdownload&host=example.com#test"
	cfg, err := ParseVLESS(raw)
	if err != nil {
		t.Fatal(err)
	}
	rebuilt := cfg.ToShareURL()
	if !strings.Contains(rebuilt, "@[2001:db8::1]:443?") {
		t.Fatalf("rebuilt URL did not bracket IPv6 address: %s", rebuilt)
	}
	parsed, err := ParseVLESS(rebuilt)
	if err != nil {
		t.Fatal(err)
	}
	if parsed.Address != "2001:db8::1" || parsed.Port != 443 {
		t.Fatalf("parsed endpoint = %s:%d", parsed.Address, parsed.Port)
	}
}
