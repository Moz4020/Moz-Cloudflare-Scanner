...
[![CI](https://github.com/moz/moz-cloudflare-scanner/actions/workflows/ci.yml/badge.svg)](https://github.com/moz/moz-cloudflare-scanner/actions/workflows/ci.yml)
[![Release](https://img.shields.io/github/v/release/moz/moz-cloudflare-scanner?style=flat-square)](https://github.com/moz/moz-cloudflare-scanner/releases/latest)
[![Go Version](https://img.shields.io/github/go-mod/go-version/moz/moz-cloudflare-scanner?style=flat-square)](go.mod)
...
Run `moz-cloudflare-scanner` and you land in a short menu. Navigate with arrow keys and Enter — no flags, no subcommands.
...
Download from the [releases page](https://github.com/moz/moz-cloudflare-scanner/releases/latest).

| Platform | Architecture | File |
|---|---|---|
| Linux | x86_64 | `moz-cloudflare-scanner_linux_x86_64.tar.gz` |
| Linux | ARM64 | `moz-cloudflare-scanner_linux_arm64.tar.gz` |
| macOS | Intel | `moz-cloudflare-scanner_darwin_x86_64.tar.gz` |
| macOS | Apple Silicon | `moz-cloudflare-scanner_darwin_arm64.tar.gz` |
| Windows | x86_64 | `moz-cloudflare-scanner_windows_x86_64.zip` |

**Linux / macOS:**

stable release:
```bash
curl -fsSL https://github.com/Moz/Moz-Cloudflare-Scanner/raw/refs/heads/main/install.sh | bash
```

pre-release:
```bash
curl -fsSL https://github.com/Moz/Moz-Cloudflare-Scanner/raw/refs/heads/main/install.sh | bash -s -- --prerelease
```

**Windows (PowerShell):**
```powershell
$r = Invoke-RestMethod https://api.github.com/repos/moz/moz-cloudflare-scanner/releases/latest
$url = ($r.assets | Where-Object name -like "*windows*x86_64*").browser_download_url
Invoke-WebRequest $url -OutFile moz-cloudflare-scanner.zip
Expand-Archive moz-cloudflare-scanner.zip .
```

### From source

```bash
go install github.com/moz/moz-cloudflare-scanner/cmd/moz-cloudflare-scanner@latest
```
...
```bash
moz-cloudflare-scanner              # open the TUI
moz-cloudflare-scanner --version    # print version and exit
moz-cloudflare-scanner -v           # same
moz-cloudflare-scanner version      # same
```
...
**Live results file:** every scan writes (and keeps updating) `MozCloudflareScannerResult-YYYYMMDD-HHMMSS.txt` next to the binary or in the working directory. Open it in any editor while the scan runs to watch results arrive.
...
Cloudflare drops ICMP on their edge IPs. Moz Cloudflare Scanner validates HTTP/TLS behaviour and, for proxy configs, runs traffic through xray — closer to real VLESS/Trojan usage than ping or bare TCP.

**How is this different from warp-plus?**
Moz Cloudflare Scanner does not run a permanent proxy. It finds and validates Cloudflare IPs for **your** xray config and exports `IP:port` lists you can plug into Sing-Box, v2rayN, etc.
...
```bash
git clone https://github.com/moz/moz-cloudflare-scanner.git
cd moz-cloudflare-scanner
...
For bugs, include your OS/arch, version (`moz-cloudflare-scanner --version`), the screen you were on, and what you expected vs what happened.
